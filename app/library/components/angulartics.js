            ...
            $injector.invoke(['$location', function($location) {
                ...
                if ($injector.has('$state')) {
                    ...
                    $rootScope.$on('$stateChangeSuccess', function(event, current) {
                        //filter ignore states
                        var ignoreStates = $analytics.settings.ignoreStates;
                        if (ignoreStates && ignoreStates.length > 0) {
                            for (var i = 0; i < ignoreStates.length; i++) {
                                if (ignoreStates[i] == current.name)
                                    return;
                            }
                        }
                        ...
                        var transformStates = $analytics.settings.transformStates;
                        if (transformStates && transformStates.length > 0) {
                            for (var i = 0; i < transformStates.length; i++) {
                                var stateItem = transformStates[i];
                                if (stateItem.state == current.name) {
                                    url = stateItem.url;
                                    break;
                                }
                            }
