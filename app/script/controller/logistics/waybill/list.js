    ...
    return ['$rootScope', '$scope', '$state','$stateParams', '$modal', 'toaster', 'LogisticsService', function($rootScope, $scope, $state, $stateParams, $modal, toaster, LogisticsService) {
       ...
       // 跳转到第n页
        ...
        $scope.searchWaybill = function() {
            ...
            LogisticsService.searchWaybills($scope.condition).then(
                function(data) {
                    if (data.status == '200') {
                        $scope.loading = false;
                        ...
                        toaster.pop('error', '加载物流运单列表失败，原因：' + data.msg);
                        $scope.loading = false;
                    }
                },
                function(errResponse) {
                    $scope.loading = false;
                    console.error('Error while searching waybills.');
                }
            );
        };
        // 进入页面第一次查询
        ...
        //跳转到运单订舱列表浏览页面
        $scope.viewWaybill=function(trackId, orderId){
        ...
        $scope.testCrossDomain=function(){
            //$scope.isSaving = true;
            LogisticsService.testCrossDomain().then(
                            function(data){
                                if (data.status=='200') {
                                     toaster.pop('success', '跨域访问成功！');
                                     //$scope.isSaving = false;
                                     //$scope.back();
                                }else{
                                    toaster.pop('error', '跨域访问失败，原因：' + data.msg);
                                    $scope.isSaving = false;
                                }
                            },
                            function(errResponse){
                                 console.error('Error while test crossdomain.');
                                //$scope.isSaving = true;
                            }
                        );
        };
