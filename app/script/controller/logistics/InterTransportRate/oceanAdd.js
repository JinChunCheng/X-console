define(['service/config', 'common/path-helper'], function(config, ph) {
    return ['$rootScope', '$scope', '$state', '$stateParams', 'LogisticsService', 'toaster','$modal', function($rootScope, $scope, $state, $stateParams, LogisticsService, toaster,$modal) {
        ...
        $scope.loadWeightIntervalInfo=function(){
            $scope.weightIntervalparam.transportMode=$scope.transportRateInfo.transportMode;
            $scope.weightIntervalparam.logisticsVendor=$scope.transportRateInfo.vendorCode;
            $scope.weightIntervalparam.calUnit=$scope.transportRateInfo.calUnit;
        ...
        $scope.loadStatesInfoBychange=function(type){
       ...
       //加载物流供应商信息列表
        ...
        //保存运费信息
        $scope.addInterTranRateInfo=function(){
            $scope.isSaving = true;
            LogisticsService.addInterTransportRateInfo($scope.transportRateInfo).then(
                function(data) {
                    if (data.status == '200') {
                        $scope.isSaving = false;
                ...
                function(errResponse) {
                    console.error('Error while add InterTransportRate.');
                }
            );
        };
    }];
