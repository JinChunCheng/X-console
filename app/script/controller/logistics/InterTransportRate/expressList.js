define(['common/session', 'service/config', 'common/path-helper'], function(session, config, ph) {
    return ['$rootScope', '$scope', '$state','$stateParams', '$modal', 'toaster', 'LogisticsService', function($rootScope, $scope, $state, $stateParams,$modal, toaster, LogisticsService) {
        ...
        ph.mark($rootScope, {
            state: 'logistics.InterTransportRate.expressList',
            title: '国际快递费率'
        });
        ...
        $scope.destCitysInfo=[];
        //$scope.weightInterval=[{id:'A1',name:'0 ~ 45KG'},{id:'A2',name:'45 ~ 200KG'}];
       ...
       // 跳转到第n页
        ...
        $scope.loadPortInfo=function(type){
            if($scope.condition.transportMode!='')
            {
                $scope.condition.serviceType='DDU';
        ...
        $scope.transportChange=function(transportMode){
            //$scope.loadPortInfoByTransport(transportMode);
            $scope.loadCalUnitInfo();
        };


        //查询国际费率信息
        $scope.searchInterExpressTransportRate = function() {
            if(!validCondition())
            {
                return false;
            }
            ...
            LogisticsService.searchInterExpressTransportRateInfo($scope.condition).then(
                function(data) {
                    if (data.status == '200') {
                        $scope.loading = false;
        ...
        //测试用预设条件
        // $scope.condition.transportMode='EXPRESS';
        // $scope.condition.serviceType='DDU';
        // $scope.condition.logisticsVendor='DHL';
        // $scope.condition.originCountryCode='CN';
        // $scope.condition.originCityCode='CN000155';
        // $scope.condition.destCountryCode='CN';
        // $scope.condition.destCityCode='CN000224';
        // $scope.searchInterExpressTransportRate();

        //跳转到编辑页面
        $scope.editExpressTransportRate=function(expresschargeId){
            $state.go('logistics.InterTransportRate.expressEdit',{
                expresschargeId:expresschargeId,
                condition:$scope.condition
            });
        };

        $scope.addExpressTransportRate=function(){
            $state.go('logistics.InterTransportRate.expressAdd',{
                condition:$scope.condition
            });
        };

        //删除快递运费
        $scope.deleteRate=function(expresschargeId){
            ...
            $modal.open({
                ...
                controller: function($scope, $modalInstance) {
                    ...
                    $scope.ok = function() {
                        deleteExpressTransportRate(expresschargeId, $scope, $modalInstance);
                        return true;
                    }
                }
            });
        ...
        var deleteExpressTransportRate=function(expresschargeId,confirmScope, modalInstance){
            confirmScope.confirmData.processing = true;
            console.debug("进来了！");
            LogisticsService.deleteInterExpressTransportRateInfo(expresschargeId).then(
                function(data) {
                    if (data.status == '200') {
                ...
                function(errResponse) {
                    console.error('Error while delete InterExpressTransportRateInfo.');
                    confirmScope.confirmData.processing = false;
                    modalInstance.dismiss();
                }
            );
