...
define(['service/config'], function(config) {
    return [
        ...
        ["CurrencyFilter", [function() {
            var filterfun = function(total, status) {
                var result = '';
                switch (status) {
                    case 'CNY':
                        result = "人民币";
        ...
        ["SupplierSkuNoFilter", function() {
            var filterfun = function(skuNo) {
                var result = "";
                if (!skuNo) {
                    return "";
                }
                /*        var skuNoArray = skuNo.split(",");
                        for(var i=0;i<skuNoArray.length;i++){
                            result += (skuNoArray[i]+",<br/>");
                        }*/
                return skuNo.replace(/,/g, "\n");
            };
            return filterfun;
        }],
        ['trackFilePath', [function() {
            //物流文件超链接增加前缀
            return function(path) {
                if (path) {
                    var host = config.img_logistics_domain;
                    if (!path || path.indexOf('http://') > -1)
                        return path;
                    return host + '/' + path;
                } else
